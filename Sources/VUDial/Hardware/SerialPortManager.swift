...
public class SerialPortManager: NSObject, ObservableObject {
    ...
    public func sendCommand(_ command: String) -> Data? {
        // ...
        ...
        let resultBox = ResultBox<Data>() // Use a thread-safe box for the result

        Task {
            let result = await sendCommandAsync(command)
    ...
    /// Thread-safe box for capturing results in Tasks
    ...
    /// - Parameter command: Command string to send
    /// - Returns: Response data or nil on timeout/error
    public func sendCommandAsync(_ command: String) async -> Data? {
        // ...
...
// SerialPortManager implements its own thread safety via NSLock and internal queues
extension SerialPortManager: @unchecked Sendable {}
