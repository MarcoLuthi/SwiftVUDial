...
@MainActor
public class DialManager: ObservableObject {
    ...
    private let updateInterval: UInt64 = 200_000_000  // 200ms in nanoseconds

    // MARK: - Initialization

    public init(serialManager: SerialPortManager, modelContext: ModelContext) {
    ...
    public func scanForDials() async {
    ...
    public func setDialValue(_ dial: Dial, value: Double) {
        dial.setValue(value)
        queueUpdate(for: dial, type: .value)
    }

    /// Set dial backlight (queued update)
    public func setDialBacklight(_ dial: Dial, red: Double, green: Double, blue: Double) {
        dial.setBacklight(red: red, green: green, blue: blue)
        queueUpdate(for: dial, type: .backlight)
    }

    /// Upload image to dial
    public func uploadImage(_ dial: Dial, image: NSImage) async {
    ...
    private func startUpdateLoop() {
        ...
        updateTask = Task { [weak self] in
            while !Task.isCancelled {
                try? await Task.sleep(nanoseconds: self?.updateInterval ?? 200_000_000)
